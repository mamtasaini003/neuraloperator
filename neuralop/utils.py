...
def configure_gpu(gpu_id=0, memory_limit=None, enable_memory_growth=True):
    """
    Configure TensorFlow GPU settings for training
    
    Args:
        gpu_id (int): GPU device ID to use (0 or 1 for your RTX A6000 setup)
        memory_limit (int, optional): Memory limit in MB. If None, uses memory growth
        enable_memory_growth (bool): Enable incremental memory allocation
    """
    gpus = [device for device in get_devices() if device.device_type == 'GPU']
    if not gpus:
        raise RuntimeError("No GPU devices found.")
    if gpu_id >= len(gpus):
        raise ValueError(f"Invalid GPU ID: {gpu_id}. Available GPUs: {len(gpus)}")
    gpu = gpus[gpu_id]
    try:
        if enable_memory_growth:
            tf.config.experimental.set_memory_growth(gpu, True)
            print(f"Enabled memory growth for GPU {gpu_id}: {gpu.name}")
        elif memory_limit is not None:
            tf.config.experimental.set_virtual_device_configuration(
                gpu,
                [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=memory_limit)]
            )
            print(f"Set memory limit for GPU {gpu_id}: {gpu.name} to {memory_limit} MB")
        else:
            print(f"Using GPU {gpu_id}: {gpu.name} with default settings")
    except RuntimeError as e:
        print(f"Error configuring GPU {gpu_id}: {e}")
        raise RuntimeError(f"Failed to configure GPU {gpu_id}.") from e
    
def select_gpu(gpu_id=0):
    """
    Alternative method using CUDA_VISIBLE_DEVICES environment variable
    Call this before importing tensorflow
    """
    os.environ['CUDA_VISIBLE_DEVICES'] = str(gpu_id)
    print(f"CUDA_VISIBLE_DEVICES set to GPU {gpu_id}")
